...
fn main() -> Result<()> {
    ...
    let initial_latents = tensors.get("initial_latents").context("Missing initial_latents")?;
    let py_packed_latents = tensors.get("packed_latents").context("Missing packed_latents")?;
    
    let rust_packed_latents = LtxPipeline::pack_latents(initial_latents, 1, 1)?;
    compare_tensors("Pack Latents", &rust_packed_latents, py_packed_latents, 1e-6)?;
    ...
    let prompt_mask = tensors.get("prompt_attention_mask").context("prompt_attention_mask")?;
    let negative_prompt_embeds = tensors.get("negative_prompt_embeds").context("negative_prompt_embeds")?;
    let negative_prompt_mask = tensors.get("negative_prompt_attention_mask").context("negative_prompt_attention_mask")?;
    ...
    let latent_model_input = Tensor::cat(&[py_packed_latents.clone(), py_packed_latents.clone()], 0)?;
    ...
    scheduler.set_timesteps(None, &device, Some(&sigmas_vec[0..n]), None, Some(&timesteps_vec))?;

    let mut current_latents = py_packed_latents.clone();
    let guidance_scale = 3.0f32;

    for (i, &t) in timesteps_vec.iter().enumerate().take(n) {
        let latent_model_input = Tensor::cat(&[current_latents.clone(), current_latents.clone()], 0)?;
        let timestep = Tensor::full(t, (2,), &device)?;
        ...
        let noise_pred_cfg = noise_uncond.broadcast_add(&diff.affine(guidance_scale as f64, 0.0)?)?;

        current_latents = scheduler.step(&noise_pred_cfg, t, &current_latents, None)?.prev_sample;
        
        let py_res = tensors.get(&format!("latents_step_{}", i)).context("Missing step result")?;
