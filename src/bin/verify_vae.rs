...
fn main() -> Result<()> {
    ...
    if let Some(ref t) = temb {
        println!("temb shape: {:?}, value: {:?}", t.dims(), t.to_vec1::<f32>());
    } else {
        println!("temb: None");
    }
    ...
    if max_diff < 0.5 { // Tolerance for 3D VAE - float precision can accumulate
         println!("\nSUCCESS: Rust VAE matches Python VAE within tolerance!");
         println!("Rust Mean: {:.4}", rust_decoded.mean_all()?.to_scalar::<f32>()?);
         println!("Py Mean: {:.4}", python_decoded.mean_all()?.to_scalar::<f32>()?);
    } else {
