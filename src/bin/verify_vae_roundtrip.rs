...
fn main() -> Result<()> {
    ...
    let verification_data = candle_core::safetensors::load("vae_roundtrip_verification.safetensors", &device)?;
    let video_input = verification_data.get("video_input").expect("Missing video_input").clone();
    let py_latents = verification_data.get("latents").expect("Missing latents").clone();
    let py_output = verification_data.get("video_output").expect("Missing video_output").clone();
    ...
    if mad_latents < 0.05 && mad_output < 0.1 {
        ...
        println!("Rust Output Mean: {:.4}", rust_output.mean_all()?.to_scalar::<f32>()?);
        println!("Py Output Mean: {:.4}", py_output.mean_all()?.to_scalar::<f32>()?);
    }
